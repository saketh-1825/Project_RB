...
def emit_event(
    analysis_id: str, event_type: str, node: str, status: str, payload: dict[str, Any]
) -> None:
    """
    ...
    except Exception as e:  # noqa: BLE001  # best-effort telemetry emission, must not crash the pipeline
        logger.error(
            f"Event system failure: Failed to broadcast event via WebSocket: {e}"
        )
