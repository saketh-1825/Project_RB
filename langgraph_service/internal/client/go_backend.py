...
class GoBackendClient:
    ...
    def get_logs(
        ...
        while True:
            params: dict[str, Any] = {"from": current_from, "to": to_time}
            ...
            try:
                response = self.client.get("/api/v1/logs", params=params)
            except httpx.TimeoutException as e:
    ...
    def get_log_anomalies(
        self, from_time: str, to_time: str, services: list[str] | None = None
    ) -> dict[str, Any]:
        ...
        response = self._request("GET", "/api/v1/logs/anomalies", params=params)
        return response.json()
    ...
    def get_trace(self, trace_id: str) -> dict[str, Any]:
        response = self._request("GET", f"/api/v1/traces/{trace_id}")
        return response.json()
    ...
    def search_runbooks(self, query: str, top_k: int = 5) -> dict[str, Any]:
        ...
        response = self._request("GET", "/api/v1/runbooks/search", params=params)
        return response.json()
    ...
    def query_metrics_batch(self, queries: list[dict[str, Any]]) -> dict[str, Any]:
        response = self._request(
            "POST", "/api/v1/metrics/query/batch", json={"queries": queries}
        )
        return response.json()
    ...
    def create_incident(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        response = self._request("POST", "/api/v1/incidents", json=data)
        return response.json()

    def get_incident(self, incident_id: str) -> dict[str, Any]:

        response = self._request("GET", f"/api/v1/incidents/{incident_id}")
        return response.json()

    def patch_incident(self, incident_id: str, data: dict[str, Any]) -> dict[str, Any]:

        response = self._request("PATCH", f"/api/v1/incidents/{incident_id}", json=data)
        return response.json()

    def post_finding(self, incident_id: str, finding: dict[str, Any]) -> dict[str, Any]:

        response = self._request(
            "POST", f"/api/v1/incidents/{incident_id}/events", json=finding
        )
        return response.json()
    ...
    def submit_report(self, incident_id: str, report: dict[str, Any]) -> dict[str, Any]:
        ...
        response = self._request(
            "POST", f"/api/v1/incidents/{incident_id}/report", json=report
        )
        return response.json()
