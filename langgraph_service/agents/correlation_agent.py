...
def _get_metric_stats(
    series: dict[str, Any] | None, scale_to_percentage: bool = False
) -> dict[str, Any]:
...
def analyze_evidence_quality(
    ...
    if not log_findings and state.get("findings"):
        log_findings = [
            f for f in state.get("findings", []) if f.get("agent") == "log_query_agent"
        ]
    ...
    if has_logs:
        ...
        missing_sources.append(
            {"source": "logs", "impact": "Cannot verify application errors from logs"}
        )
    ...
    has_metrics = (
        ...
        and not metrics_query_failed
        and len(series_list) > 0
    )
    if has_metrics:
        ...
        missing_sources.append(
            {"source": "metrics", "impact": "Cannot verify resource anomalies"}
        )
    ...
    if not rag_findings and state.get("findings"):
        rag_findings = [
            f for f in state.get("findings", []) if f.get("agent") == "rag_agent"
        ]
    ...
    if has_rag:
        ...
        missing_sources.append(
            {"source": "rag", "impact": "No historical runbooks found"}
        )
    ...
    has_topology = (
        topology_status == "success" and topology and topology.get("services")
    )
    if has_topology:
        ...
        missing_sources.append(
            {"source": "topology", "impact": "Cannot verify service dependencies"}
        )

    # Check Conflicts
    has_error_logs = any(
        f.get("type") == "log_anomaly" and not f.get("degraded") for f in log_findings
    )
...
def calculate_risk(
    evidence: dict, root_cause: dict, affected_services: list, alert: dict
) -> dict:
    """
    ...
    is_payment = any(
        "payment" in s.lower() or "customer" in s.lower() for s in affected_services
    )
    ...
    error_rate = 0.0
    for s in series_list:
        if isinstance(s, dict) and s.get("metric_name") in [
            "error_rate",
            "http_error_rate",
        ]:
            vals = [
    ...
    has_confirmed_rc = (
        root_cause
        and root_cause.get("type") != "UNKNOWN"
        and root_cause.get("confidence", 0.0) >= 0.75
    )
...
def calculate_confidence(state: AnalysisState, evidence: dict) -> dict:
    ...
    # 1. Logs Check (+0.3)
    log_status = (
        evidence.get("metadata", {}).get("collection_status", {}).get("logs", "success")
    )
    log_findings = evidence.get("logs", {}).get("findings", [])
    if not log_findings and state.get("findings"):
        log_findings = [
    ...
    if metrics_status == "success" and not metrics_query_failed:
        ...
        series_list = metrics_response.get("series", []) if metrics_response else []
        metrics_data = {
            s.get("metric_name"): s for s in series_list if isinstance(s, dict)
        }
    ...
    rag_status = (
        evidence.get("metadata", {}).get("collection_status", {}).get("rag", "success")
    )
    rag_findings = evidence.get("rag", {}).get("findings", [])
    if not rag_findings and state.get("findings"):
        rag_findings = [
    ...
    has_similar_runbook = any(
        f.get("type") == "runbook" and f.get("similarity_score", 0.0) >= 0.7
...
def _correlation_agent_node_impl(state: AnalysisState) -> AnalysisState:
    ...
    if not evidence:
        if findings:
            correlation_findings = [
                f for f in findings if f.get("agent") == "correlation_agent"
            ]
        ...
        base_url = os.environ.get(
            "GO_BACKEND_URL", "http://mock-go-backend:8080/api/v1"
        )
        ...
        if time_window and isinstance(time_window, dict):
            from_time_str = (
                time_window.get("from")
                or time_window.get("from_time")
                or time_window.get("start")
            )
            to_time_str = (
                time_window.get("to")
                or time_window.get("to_time")
        ...
        if not from_time_str or not to_time_str:
            ...
            to_time_str = (
                fired_at.astimezone(timezone.utc).isoformat().replace("+00:00", "Z")
            )
        ...
        try:
            ...
            incidents_resp = client.get_incidents(
                from_time=from_30d_str, to_time=to_time_str
            )
            past_incidents = incidents_resp.get("incidents", [])
            similar_past_incidents = find_historical_matches(
                past_incidents, affected_services
            )
        ...
        if metrics_query_failed:
            ...
            metrics_summary = {
                "cpu": _get_metric_stats(
                    metrics_data.get("cpu"), scale_to_percentage=True
                ),
        ...
        state["correlation"] = {
            "root_cause": {
                "description": correlation_finding.get(
    ...
    if metrics_query_failed:
        ...
        time_range = {"from": str(from_time_str), "to": str(to_time_str)}
        correlation_finding = build_correlation_finding(
            root_cause=root_cause,
            metric_names=["error_rate", "cpu", "memory", "db_pool_waiting"],
            time_range=time_range,
    ...
    impact = (
        f"Potential service disruption affecting {', '.join(affected_services)}"
        if affected_services
        else "Unknown service impact"
    )
    ...
    # LLM reasoning step — enriches the root cause description
    # when confidence is ambiguous (between 0.4 and 0.75).
    # Falls back to deterministic description if LLM is unavailable.
    if 0.4 <= confidence.get("score", 0.0) < 0.75:
        findings_summary = ""
        for f in state.get("findings", []):
            if f.get("agent") != "correlation_agent":
...
Write only the 2 sentences. No preamble."""
        ...
        llm_reasoning = call_llm(llm_prompt, max_tokens=400)
        if llm_reasoning:
            root_cause["llm_reasoning"] = llm_reasoning
            root_cause["description"] = llm_reasoning
            logger.info("Root cause enriched by LLM reasoning")
    ...
    state["correlation"] = {
        ...
        "supporting_evidence": {
            ...
            "topology": topology.get("services", [])
            if isinstance(topology, dict)
            else [],
        },
        "risk_level": {"severity": severity, "impact": impact},
    ...
    event = {
        ...
        "details": correlation_finding,
        "data": {"risk_level": risk_assessment, "evidence_quality": evidence_quality},
    }
    if "incident_events" not in state or not isinstance(state["incident_events"], list):
        state["incident_events"] = []

    if not any(
        e.get("source") == "correlation_agent" for e in state["incident_events"]
    ):
        state["incident_events"].append(event)
